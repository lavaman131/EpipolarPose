...
def parse_args():
    ...
    parser.add_argument(
        "--workers", help="num of dataloader workers", type=int, default=8
    )
...
def main():
    ...
    criterion = loss_fn(
        num_joints=config.MODEL.NUM_JOINTS, norm=config.LOSS.NORM
    ).cuda()
    ...
    if config.MODEL.RESUME:
        ...
        if "epoch" in checkpoint.keys():
            ...
            logger.info(
                "=> resume from pretrained model {}".format(config.MODEL.RESUME)
            )
        else:
    ...
    for epoch in range(config.TRAIN.BEGIN_EPOCH, config.TRAIN.END_EPOCH):
        ...
        perf_indicator = (
            500.0 - acc
            if config.DATASET.DATASET == "h36m" or "mpii_3dhp" or "jta"
            else acc
        )
