...
def weighted_smooth_l1_loss(input, target, weights, size_average, norm=False):
    ...
    diff = input - target
    abs = torch.abs(diff)
    out = torch.where(abs < 1.0, 0.5 * diff**2, abs - 0.5)
...
def generate_3d_integral_preds_tensor(heatmaps, num_joints, x_dim, y_dim, z_dim):
    ...
    accu_x = (
        ...
        * torch.cuda.comm.broadcast(
            torch.arange(float(x_dim)), devices=[accu_x.device.index]
        )[0]
    )
    accu_y = (
        accu_y
        * torch.cuda.comm.broadcast(
            torch.arange(float(y_dim)), devices=[accu_y.device.index]
        )[0]
    )
    accu_z = (
        accu_z
        * torch.cuda.comm.broadcast(
            torch.arange(float(z_dim)), devices=[accu_z.device.index]
        )[0]
    )
...
def softmax_integral_tensor(
    preds, num_joints, output_3d, hm_width, hm_height, hm_depth
):
    # global soft max
...
class L2JointLocationLoss(nn.Module):
    ...
    def forward(self, preds, *args):
        ...
        pred_jts = softmax_integral_tensor(
            preds, self.num_joints, self.output_3d, hm_width, hm_height, hm_depth
        )

        _assert_no_grad(gt_joints)
        _assert_no_grad(gt_joints_vis)
        return weighted_mse_loss(
            pred_jts, gt_joints, gt_joints_vis, self.size_average, self.norm
        )
...
class L1JointLocationLoss(nn.Module):
    ...
    def forward(self, preds, *args):
        ...
        pred_jts = softmax_integral_tensor(
            preds, self.num_joints, True, hm_width, hm_height, hm_depth
        )
...
def get_joint_location_result(patch_width, patch_height, preds):
    ...
    pred_jts = softmax_integral_tensor(
        preds, num_joints, True, hm_width, hm_height, hm_depth
    )
