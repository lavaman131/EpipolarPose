...
from lib.dataset.JointIntegralDataset import (
    JointsIntegralDataset,
    H36M_NAMES,
    MPII_NAMES,
)
...
H36M_TO_MPII_PERM = np.array(
    [H36M_NAMES.index(h) for h in MPII_NAMES if h != "" and h in H36M_NAMES]
)
...
class Rat7M_Integral(JointsIntegralDataset):
    ...
    def __getitem__(self, idx):
        if self.is_train and self.cfg.DATASET.TRI:
            ...
            cam_2 = (
                self.cam_config[cam_1][0]
                if random.random() <= 0.5
                else self.cam_config[cam_1][1]
            )
    ...
    def get_data(self, the_db):
        ...
        img_patch, label, label_weight, scale, rot = get_single_patch_sample(
            image_file,
            the_db["center_x"],
            the_db["center_y"],
            the_db["width"],
            ...
            joints_vis,
            the_db["flip_pairs"].copy(),
            the_db["parent_ids"].copy(),
            self.patch_width,
            self.patch_height,
            self.rect_3d_width,
            self.rect_3d_height,
        ...
        return (
            img_patch.astype(np.float32),
            label.astype(np.float32),
            label_weight.astype(np.float32),
            meta,
        )

    def _get_train_db(self):
        # create train/val split
        file_name = os.path.join(self.root, "annot", self.image_set + ".pkl")
        ...
        if isinstance(anno, dict):
            ...
            ]  # for each cameras construct a database
    ...
    def evaluate(self, preds, save_path=None, debug=False, actionwise=False):
        ...
        j14 = (
            ...
            else [0, 1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12, 14, 15]
        )
        ...
        for n_sample in range(0, sample_num):
            ...
            _, Z, T, b, c = compute_similarity_transform(
                gt_3d_kpt, pre_3d_kpt, compute_optimal_scale=True
            )
            ...
            if self.cfg.DEBUG.DEBUG:
                ...
                drawskeleton(
                    img, pre_2d_kpt, thickness=3, lcolor=lc[0], rcolor=rc[0], mpii=m
                )
                drawskeleton(
                    img,
                    gt_2d_kpt,
            ...
            pred_to_save.append(
                {"pred": pre_3d_kpt, "align_pred": pre_3d_kpt_align, "gt": gt_3d_kpt}
            )
