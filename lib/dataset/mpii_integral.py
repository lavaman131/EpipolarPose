...
class MPIIDataset(JointsIntegralDataset):
    ...
    def __getitem__(self, idx):
        ...
        img_patch, label, label_weight, _, _ = get_single_patch_sample(
            the_db["image"],
            the_db["center_x"],
            the_db["center_y"],
            the_db["width"],
    ...
    def _get_db(self):
        ...
        for a in anno:
            # joints and vis
            jts_3d = np.zeros((self.num_joints, 3), dtype=np.float64)
            jts_3d_vis = np.zeros((self.num_joints, 3), dtype=np.float64)
            if self.image_set != "test":
                jts = np.array(a["joints"])
                jts[:, 0:2] = jts[:, 0:2] - 1
                jts_vis = np.array(a["joints_vis"])
                assert len(jts) == self.num_joints, "joint num diff: {} vs {}".format(
                    len(jts), self.num_joints
                )
                jts_3d[:, 0:2] = jts[:, 0:2]
